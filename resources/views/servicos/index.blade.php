                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
                        ...
                        <div class="flex flex-col gap-4 md:flex-row md:items-center">
                            <form method="GET" action="{{ route('servicos.index') }}" class="flex items-center">
                                <input
                                    type="text"
                                    name="q"
                                    value="{{ $busca }}"
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Tipo de cobrança</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Preço vigente</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($servicos as $servico)
                                    @php
                                        $precoAtual = $servico->prices->first();
                                    @endphp
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $servico->codigo }}</td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100">
                                            {{ $servico->nome }}
                                            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ $servico->descricao ?? '-' }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                            {{ ucfirst($servico->tipo_cobranca) }} ({{ strtoupper($servico->unidade_medida) }})
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                            @if ($precoAtual)
                                                R$ {{ number_format($precoAtual->preco, 2, ',', '.') }}
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-3">
                                            <a href="{{ route('servicos.show', $servico) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300">Ver</a>
                                            <a href="{{ route('servicos.edit', $servico) }}" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-900 dark:hover:text-yellow-300">Editar</a>
